...
const filePath = path.join(__dirname, 'frontend', 'src', 'pages', 'AdminPortal.jsx');
...
content = content.replace(/const Sidebar = \(\) => \(\s*<div className="w-full md:w-72 h-auto md:h-screen sticky top-0 shrink-0 z-20 p-2 md:p-4 flex flex-col">\s*<div className="bg-white rounded-\[24px\] shadow-\[0_4px_20px_rgba\(0,0,0,0.03\)\] flex flex-row md:flex-col h-full border border-slate-100 overflow-x-auto md:overflow-hidden">/, 
`const Sidebar = () => (
    <div className="w-full md:w-72 h-[100dvh] shrink-0 z-20 p-4 flex flex-col overflow-y-auto custom-scrollbar">
      <div className="bg-white rounded-[24px] shadow-[0_4px_20px_rgba(0,0,0,0.03)] flex flex-col h-full border border-slate-100 overflow-hidden">`);
...
// Fix hidden text for label
content = content.replace(/<span className="hidden sm:inline md:inline">\{item.label\}<\/span>/, '<span>{item.label}</span>');
...
`<div className="flex flex-col md:flex-row w-full h-[100dvh] bg-[#f4f7fe] overflow-hidden font-sans relative">
  ...
  <div className="md:hidden flex items-center justify-between p-4 bg-white border-b border-slate-200 z-30 shrink-0 shadow-sm">
     <div className="flex items-center gap-3">
       ...
       <span className="font-black text-slate-800 text-lg">JNTUGV Admin</span>
     </div>
