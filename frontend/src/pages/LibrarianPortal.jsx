...
export default function LibrarianPortal() {
  ...
  const handleSendNotification = async (e) => {
    ...
    try {
      await axios.post('/api/notifications', {
        recipient_id: notifTarget === 'all' ? null : notifTarget,
        title: notifTitle,
  ...
  const handleAppealStatus = async (id, status) => {
    try {
      ...
      <div className="bg-white/40 backdrop-blur-3xl rounded-[24px] shadow-[0_4px_24px_rgba(0,0,0,0.02)] border border-white/50 flex flex-row md:flex-col h-full overflow-x-auto md:overflow-hidden">
        ...
        <div className="px-5 pb-4 hidden md:block">
          <div className="bg-[#f4f7fe] rounded-2xl p-3 flex items-center gap-3 border border-slate-100">
            ...
            <div>
              <p className="text-xs font-black text-slate-800">{user?.name || 'Head Librarian'}</p>
              <p className="text-[10px] font-bold text-slate-400 capitalize">Librarian</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-1 shrink-0 z-10 w-full md:w-auto text-center md:text-left">
        ...
        <div className="flex items-center justify-center md:justify-start gap-3 mt-2 bg-white rounded-full py-1.5 pl-1.5 pr-5 border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] w-fit mx-auto md:mx-0 hover:shadow-md transition-all cursor-pointer">
          <div className="w-8 h-8 rounded-full overflow-hidden shrink-0">
            <img src={localPhoto || user?.profile_photo || "https://images.unsplash.com/photo-1544717305-2782549b5136?auto=format&fit=facearea&facepad=2&w=256&h=256&q=80"} alt="Profile" className="w-full h-full object-cover" />
          </div>
          <span className="text-sm font-bold text-slate-700">{user?.name || 'Head Librarian'}</span>
        </div>
      </div>
        ...
        <div className="p-4 md:p-6 max-w-7xl mx-auto w-full relative pb-20">
          ...
          {activeTab === 'dashboard' && (
            <div className="space-y-6 animate-in fade-in duration-300">
              {/* Verified Access Pill (from mockup, floating right below topbar) */}
              ...
              <div className="bg-gradient-to-r from-[#d9e8fc] to-[#e8f1fd] p-10 rounded-3xl shadow-[0_4px_24px_rgba(0,0,0,0.02)] border border-blue-100 flex flex-col md:flex-row items-center justify-between overflow-hidden relative mt-4">
                <div className="z-10 relative max-w-lg">
                  <h2 className="text-3xl font-black text-slate-800 mb-2 tracking-tight">Librarian Control Hub</h2>
                ...
                <div className="hidden md:block absolute right-[-5%] top-0 bottom-0 w-1/2">
                  ...
                  <Library className="absolute right-16 top-1/2 -translate-y-1/2 w-48 h-48 text-blue-500/20 drop-shadow-xl" strokeWidth={1} />
                </div>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
                {[
                  { label: 'LIVE PORTAL USERS', value: stats?.liveUsers || 0, color: 'text-blue-500', icon: <Users className="w-5 h-5 text-blue-500" />, bg: 'bg-blue-50' },
                  { label: 'TRANSACTING TODAY', value: stats?.transactingUsers || 0, color: 'text-purple-500', icon: <ArrowLeftRight className="w-5 h-5 text-purple-500" />, bg: 'bg-purple-50' },
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                {/* Top Subjects Chart */}
                <div className="bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-slate-100 p-6 flex flex-col items-center">
                  <div className="w-full flex justify-between items-center mb-6">
                    <h3 className="text-sm font-black text-slate-800">Top Subject Departments</h3>
                    <div className="w-6 h-6 flex flex-col gap-1 items-center justify-center cursor-pointer text-slate-400">
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
               ...
               <div className="bg-white rounded-[32px] p-8 border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] flex items-center justify-between overflow-hidden relative">
                 <div className="relative z-10">
                   <h2 className="text-[26px] font-black text-[#0f172a] tracking-tight mb-1">Add New Book Volume</h2>
                   <p className="text-slate-500 font-medium text-sm">Register dynamic print volumes and assign grid coordinate maps.</p>
                 </div>
                 <div className="hidden md:flex absolute right-0 top-0 bottom-0 w-1/3 items-center justify-end pr-10 opacity-90">
                    <BookOpen className="w-32 h-32 text-indigo-500/10" />
                 </div>
               ...
               <div className="flex flex-col xl:flex-row gap-6">
                  ...
                  <div className="flex-1 space-y-6">
                     {addMsg && <div className={`p-4 rounded-2xl font-bold text-sm ${addMsg.includes('Error') ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-green-50 text-green-600 border border-green-100'}`}>{addMsg}</div>}
                     ...
                     <div className="bg-white p-8 rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)]">
                        <div className="flex items-center gap-3 mb-8">
                           ...
                           <h3 className="text-xs font-black text-indigo-800 tracking-widest uppercase">Book Metadata Information</h3>
                        </div>
                        <div className="grid grid-cols-2 gap-6">
                           <div className="col-span-2">
                             <label className="block text-[11px] font-black text-[#0f172a] mb-2">Book Title</label>
                             <input type="text" value={newBook.title} onChange={e => setNewBook({...newBook, title: e.target.value})} className="w-full bg-white border border-slate-200 rounded-[14px] py-3.5 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none" placeholder="e.g. Introduction to Algorithms" />
                           </div>
                           <div>
                             <label className="block text-[11px] font-black text-[#0f172a] mb-2">Author Name</label>
                             <input type="text" value={newBook.author} onChange={e => setNewBook({...newBook, author: e.target.value})} className="w-full bg-white border border-slate-200 rounded-[14px] py-3.5 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none" placeholder="e.g. Thomas H. Cormen" />
                           </div>
                           <div>
                             <label className="block text-[11px] font-black text-[#0f172a] mb-2">ISBN-13 Number</label>
                             <div className="relative">
                               <input type="text" value={newBook.isbn} onChange={e => setNewBook({...newBook, isbn: e.target.value})} className="w-full bg-white border border-slate-200 rounded-[14px] py-3.5 px-4 pr-10 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none" placeholder="e.g. 978-0262033848" />
                               <div className="absolute right-4 top-1/2 -translate-y-1/2 flex gap-[3px] text-slate-300">
                                 <div className="w-[3px] h-[14px] bg-current"></div><div className="w-[3px] h-[18px] bg-current"></div><div className="w-[3px] h-[12px] bg-current"></div><div className="w-[3px] h-[20px] bg-current"></div>
                               </div>
                             </div>
                           </div>
                           <div className="col-span-2 md:col-span-1">
                             <label className="block text-[11px] font-black text-[#0f172a] mb-2">Number of Copies</label>
                             <div className="relative">
                               <input type="number" min="1" value={newBook.count} onChange={e => setNewBook({...newBook, count: e.target.value})} className="w-full bg-white border border-slate-200 rounded-[14px] py-3.5 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none" placeholder="1" />
                     ...
                     <div className="bg-white p-8 rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)]">
                        <div className="flex items-center gap-3 mb-8">
                           ...
                           <h3 className="text-xs font-black text-indigo-800 tracking-widest uppercase">Physical Location Assignment Coordinates</h3>
                        </div>
                        <div className="grid grid-cols-2 gap-6">
                           <div>
                             <label className="block text-[11px] font-black text-[#0f172a] mb-2">Physical Room</label>
                             <div className="relative">
                               <MapPin className="w-4 h-4 text-indigo-400 absolute left-4 top-1/2 -translate-y-1/2" />
                               <input type="text" value={newBook.room} onChange={e => setNewBook({...newBook, room: e.target.value})} className="w-full bg-white border border-slate-200 rounded-[14px] py-3.5 pl-11 pr-10 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none" placeholder="Room 02" />
                             </div>
                           </div>
                           <div>
                             <label className="block text-[11px] font-black text-[#0f172a] mb-2">Rack Number</label>
                             <div className="relative">
                               <Database className="w-4 h-4 text-indigo-400 absolute left-4 top-1/2 -translate-y-1/2" />
                               <input type="text" value={newBook.rack} onChange={e => setNewBook({...newBook, rack: e.target.value})} className="w-full bg-white border border-slate-200 rounded-[14px] py-3.5 pl-11 pr-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none" placeholder="e.g. 05" />
                             </div>
                           </div>
                           <div>
                             <label className="block text-[11px] font-black text-[#0f172a] mb-2">Shelf</label>
                             <div className="relative">
                               <Database className="w-4 h-4 text-indigo-400 absolute left-4 top-1/2 -translate-y-1/2" />
                               <input type="text" value={newBook.shelf} onChange={e => setNewBook({...newBook, shelf: e.target.value})} className="w-full bg-white border border-slate-200 rounded-[14px] py-3.5 pl-11 pr-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none" placeholder="e.g. 02" />
                             </div>
                           </div>
                           <div>
                             <label className="block text-[11px] font-black text-[#0f172a] mb-2">Grid / Position</label>
                             <div className="relative">
                               <Database className="w-4 h-4 text-indigo-400 absolute left-4 top-1/2 -translate-y-1/2" />
                               <input type="text" placeholder="e.g. B3" className="w-full bg-white border border-slate-200 rounded-[14px] py-3.5 pl-11 pr-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none" />
                             </div>
                           </div>
                        </div>
                     </div>
                  ...
                  {/* Right Column (Generators & Actions) */}
                  <div className="w-full xl:w-[360px] shrink-0 flex flex-col gap-6">
                     <div className="bg-white p-7 rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] flex flex-col items-center">
                        <div className="flex items-center gap-3 mb-6 w-full">
                           <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center shrink-0">
                              <QrCode className="w-4 h-4 text-indigo-600"/>
                           </div>
                           <h3 className="text-[11px] font-black text-indigo-800 tracking-widest uppercase">Codes Generator Tools</h3>
                        </div>
                        
                        <div className="flex gap-2 w-full bg-slate-50/50 p-1.5 rounded-xl mb-8 border border-slate-100">
                           <button className="flex-1 py-2.5 bg-transparent text-slate-500 font-bold text-xs rounded-[10px] hover:text-slate-800 transition-all">Barcode</button>
                           <button className="flex-1 py-2.5 bg-white text-indigo-600 shadow-sm border border-indigo-100/50 rounded-[10px] text-xs font-black transition-all">QR Code</button>
                        </div>
                        
                        <div className="w-full aspect-square bg-white border-2 border-dashed border-slate-200 rounded-[20px] flex flex-col items-center justify-center mb-8 relative">
                           <QrCode className="w-[120px] h-[120px] text-indigo-300/40 mb-6" strokeWidth={1} />
                           <span className="text-[11px] font-black text-slate-400 tracking-widest uppercase absolute bottom-6">QR Code Preview</span>
                        </div>
                        
                        <button onClick={handleAddBook} className="w-full bg-gradient-to-r from-[#6366f1] to-[#a855f7] text-white font-bold py-4 rounded-[14px] shadow-lg shadow-indigo-500/30 hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 text-sm">
                           <Database className="w-4 h-4" /> Save Book to Relational DB
                        </button>
                     </div>
                     
                     <div className="bg-white p-7 rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)]">
                        <div className="flex items-center gap-3 mb-5">
                           <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center shrink-0">
                              <Download className="w-4 h-4 text-indigo-600"/>
                           </div>
                           <h3 className="text-[11px] font-black text-indigo-800 tracking-widest uppercase">Bulk Import Via CSV</h3>
                        </div>
                        <p className="text-xs text-slate-500 mb-6 leading-relaxed font-medium">Upload a CSV file with columns: Title, Author, ISBN, Copies, Section, Room, Rack, Shelf, Barcode (comma separated for multiple copies).</p>
                        <div className="relative">
                           <input type="file" accept=".csv" onChange={handleCSVUpload} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" disabled={csvLoading} />
                           <div className={`w-full py-3.5 rounded-[14px] border border-slate-200 font-bold text-xs flex justify-center items-center gap-2 transition-all shadow-sm ${csvLoading ? 'text-slate-400 bg-slate-50' : 'text-indigo-600 bg-white hover:bg-indigo-50 hover:border-indigo-200'}`}>
               ...
               <div className="bg-white rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
                  <div className="overflow-x-auto max-h-[600px]">
                    <table className="w-full text-left text-sm">
                      <thead className="bg-slate-50/50 text-slate-400 uppercase tracking-widest font-black text-[11px] sticky top-0 backdrop-blur-md">
                        <tr>
                          <th className="px-6 py-5">Title & Author</th>
                          <th className="px-6 py-5">Asset Barcode</th>
                          <th className="px-6 py-5">Location Map</th>
                        ...
                        {allBooks.map((b) => (
                          <tr key={b.id} className="hover:bg-slate-50/30 transition-colors group">
                            <td className="px-6 py-5">
                               ...
                               <p className="text-slate-500 text-xs font-medium mt-0.5">{b.author || 'Unknown'}</p>
                            </td>
                            <td className="px-6 py-5">
                               <span className="font-mono text-slate-600 font-bold bg-slate-100 px-2.5 py-1 rounded-lg text-xs">{b.asset_id}</span>
                            </td>
                            <td className="px-6 py-5">
                            ...
                            <td className="px-6 py-5 text-right flex gap-2 justify-end">
                               <button onClick={() => setLocatingBook(b)} className="text-blue-600 font-bold hover:bg-blue-50 px-3 py-2 rounded-xl text-xs transition-colors flex items-center gap-1"><Navigation className="w-3.5 h-3.5" /> Locate</button>
                               <button onClick={() => setShowQrBook(b)} className="text-emerald-600 font-bold hover:bg-emerald-50 px-3 py-2 rounded-xl text-xs transition-colors flex items-center gap-1"><QrCode className="w-3.5 h-3.5" /> QR</button>
                               <button onClick={() => setEditBook(b)} className="text-indigo-600 font-bold hover:bg-indigo-50 px-4 py-2 rounded-xl text-xs transition-colors">Edit</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
          ...
          {activeTab === 'members' && (
            <div className="space-y-6 animate-in fade-in duration-300">
               <div className="bg-white p-8 rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
                 <div className="absolute top-0 right-0 w-64 h-64 bg-fuchsia-50 rounded-full blur-3xl -mr-20 -mt-20 opacity-50 pointer-events-none"></div>
                 <div className="relative z-10">
                   <h2 className="text-2xl font-black text-slate-800 mb-2">Members & Communications</h2>
                   <p className="text-slate-500 font-medium">Manage student accounts and send library-wide or direct notifications.</p>
                 </div>
               </div>
               ...
               <div className="bg-white p-8 rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)]">
                  <div className="flex items-center gap-3 mb-8">
                     ...
                     <h3 className="text-xs font-black text-indigo-800 tracking-widest uppercase">Send Notification</h3>
                  </div>
                  <form onSubmit={handleSendNotification} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <div className="col-span-1 md:col-span-2">
                       <label className="block text-[11px] font-black text-[#0f172a] mb-2 uppercase tracking-widest">Recipient</label>
                       <select value={notifTarget} onChange={e => setNotifTarget(e.target.value)} className="w-full bg-white border border-slate-200 rounded-[14px] py-3.5 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20width%3D%2224%22%20height%3D%2224%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Cpath%20d%3D%22M6%209L12%2015L18%209%22%20stroke%3D%22%2364748B%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%2F%3E%3C%2Fsvg%3E')] bg-[length:20px] bg-[position:right_12px_center] bg-no-repeat pr-10">
                         <option value="all">Broadcast to All Students</option>
                         {allUsers.filter(u => u.role === 'student').map(u => (
                           <option key={u.id} value={u.id}>{u.name} ({u.barcode_id})</option>
                         ))}
                       </select>
                     </div>
                     <div>
                       <label className="block text-[11px] font-black text-[#0f172a] mb-2 uppercase tracking-widest">Notification Title</label>
                       <input type="text" value={notifTitle} onChange={e => setNotifTitle(e.target.value)} required placeholder="e.g. Return Due Tomorrow" className="w-full bg-white border border-slate-200 rounded-[14px] py-3.5 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none" />
                     </div>
                     <div>
                       <label className="block text-[11px] font-black text-[#0f172a] mb-2 uppercase tracking-widest">Alert Type</label>
                       <select value={notifType} onChange={e => setNotifType(e.target.value)} className="w-full bg-white border border-slate-200 rounded-[14px] py-3.5 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20width%3D%2224%22%20height%3D%2224%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Cpath%20d%3D%22M6%209L12%2015L18%209%22%20stroke%3D%22%2364748B%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%2F%3E%3C%2Fsvg%3E')] bg-[length:20px] bg-[position:right_12px_center] bg-no-repeat pr-10">
                         <option value="info">General Info (Blue)</option>
                         <option value="success">Success / Ready (Green)</option>
                         <option value="warning">Warning / Notice (Amber)</option>
                         <option value="error">Error / Urgent (Red)</option>
                       </select>
                     </div>
                     <div className="col-span-1 md:col-span-2">
                       <label className="block text-[11px] font-black text-[#0f172a] mb-2 uppercase tracking-widest">Message Body</label>
                       <textarea value={notifMessage} onChange={e => setNotifMessage(e.target.value)} required placeholder="Type the detailed message here..." rows={3} className="w-full bg-white border border-slate-200 rounded-[14px] py-3.5 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none resize-none"></textarea>
                     </div>
                     <div className="col-span-1 md:col-span-2 text-right">
                       <button type="submit" disabled={sendingNotif} className="bg-indigo-600 text-white font-bold py-3.5 px-8 rounded-[14px] shadow-md shadow-indigo-600/20 hover:bg-indigo-700 transition-all active:scale-95 disabled:opacity-50">
                         {sendingNotif ? 'Sending...' : 'Dispatch Notification'}
                       </button>
                     </div>
                  </form>
               </div>
                    ...
                    <table className="w-full text-left text-sm">
                      <thead className="bg-slate-50/50 text-slate-400 uppercase tracking-widest font-black text-[11px] sticky top-0 backdrop-blur-md">
                        <tr>
                          ...
                          <th className="px-6 py-5">Status / Fines</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100/50">
                        {allUsers.map((u) => (
                          <tr key={u.id} className="hover:bg-slate-50/30 transition-colors group">
                            <td className="px-6 py-5">
                               <p className="font-bold text-slate-800 text-sm group-hover:text-indigo-600 transition-colors">{u.name}</p>
                               <p className="text-slate-500 text-xs font-medium mt-0.5">{u.email}</p>
                            </td>
                            <td className="px-6 py-5">
                               <span className="font-mono text-slate-600 font-bold bg-slate-100 px-2.5 py-1 rounded-lg text-xs">{u.barcode_id}</span>
                            </td>
                            <td className="px-6 py-5 text-slate-500 text-[10px] font-black uppercase tracking-widest">{u.role}</td>
                            <td className="px-6 py-5">
                              <span className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${u.fines > 0 ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-600'}`}>
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
               <div className="bg-white p-8 rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
                 ...
                 <div className="relative z-10">
                   ...
                   <p className="text-slate-500 font-medium">Review and process student book reservations.</p>
                 </div>
               </div>
               
               {/* Reservations Section */}
               <div className="bg-white rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
                  <div className="bg-white border-b border-slate-100 px-8 py-6 flex items-center gap-3">
                     <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center shrink-0">
                        <Library className="w-4 h-4 text-indigo-600"/>
                     </div>
                     <h3 className="text-xs font-black text-slate-800 tracking-widest uppercase">Book Reservations <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full ml-2">{reservations.length}</span></h3>
                  </div>
                  <div className="overflow-x-auto max-h-[400px]">
                    <table className="w-full text-left text-sm">
                      <thead className="bg-slate-50/50 text-slate-400 uppercase tracking-widest font-black text-[11px] sticky top-0 backdrop-blur-md z-10">
                        <tr>
                      ...
                      <tbody className="divide-y divide-slate-100/50">
                        {reservations.map((res) => (
                          <tr key={res.reservation_id} className="hover:bg-slate-50/30 transition-colors group">
                            <td className="px-8 py-5">
                               <p className="font-bold text-slate-800 text-sm group-hover:text-indigo-600 transition-colors">{res.student_name}</p>
                               <p className="text-slate-500 text-xs font-mono mt-0.5">{res.student_barcode}</p>
                            </td>
                            <td className="px-8 py-5">
                               <p className="font-bold text-slate-800 text-sm">{res.book_title}</p>
                               <p className="text-slate-500 text-xs mt-0.5">{res.author}</p>
                            </td>
                            <td className="px-8 py-5 text-slate-500 text-xs font-medium">{new Date(res.created_at).toLocaleDateString()}</td>
                            <td className="px-8 py-5">
                              <span className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${
                                 res.status === 'Ready' ? 'bg-emerald-50 text-emerald-600' : 
                                 res.status === 'Completed' ? 'bg-indigo-50 text-indigo-600' : 
                                 res.status === 'Cancelled' ? 'bg-slate-50 text-slate-600' : 
                            ...
                            <td className="px-8 py-5 text-right space-x-2">
                               {res.status === 'Pending' && (
                                 <button onClick={() => handleReservationStatus(res.reservation_id, 'Ready')} className="text-indigo-600 font-bold hover:bg-indigo-50 text-xs px-3 py-1.5 rounded-lg transition-colors">Mark Ready</button>
                               )}
                               {res.status === 'Ready' && (
                                 <button onClick={() => handleReservationStatus(res.reservation_id, 'Completed')} className="text-emerald-600 font-bold hover:bg-emerald-50 text-xs px-3 py-1.5 rounded-lg transition-colors">Complete</button>
                               )}
                               {(res.status === 'Pending' || res.status === 'Ready') && (
                                 <button onClick={() => handleReservationStatus(res.reservation_id, 'Cancelled')} className="text-rose-600 font-bold hover:bg-rose-50 text-xs px-3 py-1.5 rounded-lg transition-colors">Cancel</button>
                               )}
                            </td>
                          </tr>
                        ))}
                        {reservations.length === 0 && (
                          <tr>
                            <td colSpan="5" className="px-8 py-12 text-center text-slate-400 font-medium">No active reservations right now.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
          ...
          {activeTab === 'appeals' && (
            <div className="space-y-6 animate-in fade-in duration-300">
               <div className="bg-white p-8 rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
                 ...
                 <div className="relative z-10">
                   <h2 className="text-2xl font-black text-slate-800 mb-2">General Appeals</h2>
                   <p className="text-slate-500 font-medium">Review student requests and fine appeals.</p>
                 </div>
               </div>
               ...
               <div className="bg-white rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden">
                  <div className="bg-white border-b border-slate-100 px-8 py-6 flex items-center gap-3">
                     ...
                     <h3 className="text-xs font-black text-slate-800 tracking-widest uppercase">General Appeals <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full ml-2">{appeals.length}</span></h3>
                  </div>
                  <div className="overflow-x-auto max-h-[400px]">
                    <table className="w-full text-left text-sm">
                      <thead className="bg-slate-50/50 text-slate-400 uppercase tracking-widest font-black text-[11px] sticky top-0 backdrop-blur-md z-10">
                        <tr>
                          <th className="px-8 py-5">Student Info</th>
                          <th className="px-8 py-5">Appeal Type</th>
                      ...
                      <tbody className="divide-y divide-slate-100/50">
                        {appeals.map((app) => (
                          <tr key={app.appeal_id} className="hover:bg-slate-50/30 transition-colors group">
                            <td className="px-8 py-5">
                               <p className="font-bold text-slate-800 text-sm group-hover:text-indigo-600 transition-colors">{app.student_name}</p>
                               <p className="text-slate-500 text-xs font-mono mt-0.5">{app.student_barcode}</p>
                            </td>
                            <td className="px-8 py-5">
                               <span className="bg-indigo-50 text-indigo-600 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest">{app.appeal_type}</span>
                            </td>
                            <td className="px-8 py-5 text-slate-600 whitespace-pre-wrap text-xs font-medium max-w-xs">{app.description}</td>
                            <td className="px-8 py-5">
                              <span className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${
                            ...
                            <td className="px-8 py-5 text-right space-x-2">
                               {app.status === 'Pending' && (
                                 <>
                                   <button onClick={() => handleAppealStatus(app.appeal_id, 'Approved')} className="text-emerald-600 font-bold hover:bg-emerald-50 text-xs px-3 py-1.5 rounded-lg transition-colors">Approve</button>
                                   <button onClick={() => handleAppealStatus(app.appeal_id, 'Rejected')} className="text-rose-600 font-bold hover:bg-rose-50 text-xs px-3 py-1.5 rounded-lg transition-colors">Reject</button>
                                 </>
                               )}
                            </td>
                          </tr>
                        ...
                        {appeals.length === 0 && (
                          <tr>
                            <td colSpan="5" className="px-8 py-12 text-center text-slate-400 font-medium">No active appeals right now.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
               ...
               <div className="bg-white p-8 rounded-[24px] border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] max-w-2xl">
                  <div className="space-y-6">
                     <div>
                        ...
                        <div className="relative">
                           <input type="text" defaultValue={user?.name || 'librarian'} className="w-full bg-slate-50 border border-slate-200 rounded-[14px] py-3.5 pl-11 pr-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none" />
