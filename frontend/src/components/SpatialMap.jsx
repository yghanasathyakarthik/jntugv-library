    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white rounded-[32px] w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl shadow-indigo-900/20">
        ...
        <div className="flex-1 bg-slate-50 p-8 relative overflow-hidden flex items-center justify-center min-h-[400px]">
          ...
          <div className="w-full max-w-[800px] aspect-[16/9] bg-white border-4 border-slate-200 rounded-3xl shadow-inner relative transform perspective-[1000px] rotate-x-[20deg] rotate-y-[-10deg] scale-95 transition-transform duration-1000 ease-out hover:rotate-x-[15deg] hover:rotate-y-[-5deg] hover:scale-100 flex">
            ...
            {/* Entrance */}
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-32 h-4 bg-emerald-400 rounded-t-lg border-2 border-emerald-500 flex items-center justify-center z-10">
               <span className="text-[10px] font-black text-white uppercase tracking-widest">Entrance</span>
            </div>

            {/* Left Side: Non-Circulating Room */}
            <div className="flex-1 border-r-4 border-slate-300 relative">
              <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/80 px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest text-slate-400">Non-Circulating (Left)</div>
              <div className="absolute inset-x-8 top-16 bottom-16 grid grid-cols-3 grid-rows-4 gap-6">
                {Array.from({ length: 12 }).map((_, i) => {
                  const isTarget = locationData?.room_number?.includes('Non-Circulating') && i === 5; // mock position
                  return (
            ...
            {/* Right Side: Circulating Room */}
            <div className="flex-1 relative">
              <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/80 px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest text-slate-400">Circulating (Right)</div>
              <div className="absolute inset-x-8 top-16 bottom-16 grid grid-cols-3 grid-rows-4 gap-6">
                {Array.from({ length: 12 }).map((_, i) => {
                  const isTarget = locationData?.room_number?.includes('Circulating') && !locationData?.room_number?.includes('Non-Circulating') && i === 5; // mock position
                  return (
                    <div key={i} className={`relative rounded-lg flex items-center justify-center transition-all duration-500 shadow-md ${
                      isTarget 
                        ? 'bg-indigo-600 border-2 border-indigo-400 shadow-xl shadow-indigo-600/40 z-20 scale-110' 
        ...
        <div className="bg-slate-800 p-6 flex flex-col md:flex-row items-center justify-between gap-4">
           <div className="flex flex-wrap gap-4">
             ...
             <div className="bg-slate-700/50 px-4 py-2 rounded-xl border border-slate-600">
               <span className="text-slate-400 text-xs font-bold uppercase tracking-wider block mb-1">Rack Name</span>
               <span className="text-white font-mono font-bold text-lg text-emerald-400">{locationData?.rack_number || 'General Rack'}</span>
             </div>
           </div>
